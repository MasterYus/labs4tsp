...
#include "lab5/5_1/division.cpp"
#include "lab5/5_2/IndexOutOfRangeException.h"
#include "lab5/5_2/IndexLowerException.h"
#include "lab5/5_2/IndexUpperException.h"
#include "lab5/5_3/EmptySetException.h"
...
void task_5_1() {
	float divisible, divider, result;
	cout << "Введите делимое и делитель через пробел: ";
	cin >> divisible >> divider;
	try {
		result = division(divisible, divider);
		cout << "Результат " << result << endl;
	}
	catch (runtime_error& e) {
		cout << "Поймали исключение!" << endl << e.what();
	}
	cout << endl;
}
...
void task_5_2() {
	...
	cout << "Массив: { 0, 1, 2, 3 }" << endl;
	cout << "Введите индекс: ";
	cin >> index;

	try {
		if (index < 0)
			throw IndexLowerException(index);
		if (index >= sizeof(array) / sizeof(int))
			throw IndexUpperException(index);
		cout << "По индексу " << index << " находится элемент " << array[index] << endl;
	}
	catch (IndexOutOfRangeException& error) {
		cout << "Ошибочка: " << error.what() << " (индекс = " << error.getIndex() << ")" << endl;
	}
}
...
void task_5_3() {
	...
	try {
		...
		string res = set.count(searching) ? "" : " не ";
		cout << "\nЭлемент " << searching << res << "найден в множестве\n" << endl;
	}
	catch (const EmptySetException& error) {
		cerr << "\nОшибочка: " << error.what() << endl;
	}
}